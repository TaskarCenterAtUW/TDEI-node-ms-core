...
async function testCloneFile() {
    await storageClient?.cloneFile("https://tdeisamplestorage.blob.core.windows.net/streams/backend-jobs/1/edges.OSW.geojson", "osw", "backend-jobs/1/edges.OSW.geojson").then((fileEntity) => {
        console.log("Received file entity");
        console.log(fileEntity.fileName);
        console.log(fileEntity.mimeType);
    });
